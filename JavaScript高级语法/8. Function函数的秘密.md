...
* 当函数默认参数的默认值为函数的时候，且函数会访问默认参数的变量的时候，会多形成一个参数作用域，当参数作用域有变量访问的时候，会优先访问参数作用域的变量，查不到按照作用域链向上查找
...
3. 传入无效值，如果传入无效值（undefined, null, false, '', 0等等）且传值为undefined，那么使用默认值
function sum (a, b = 2) {
    console.log(a); // 1
    console.log(b); // 2
    return a + b;
...
6. 有默认值的函数length属性(就是函数参数的个数)
function foo (a, b) {};
foo.length // 2 正常的函数

function bar (a, b = 1) {}
bar.length // 1 有默认参数，默认参数不算
