...
3. // 类的特点 其实了解的就知道 就是相当于 之前构造函数的语法糖 
...
```js
 // 我们说个知识点 子类 是继承 父类的this然后对其进行 进行加共在返回
...
new Student() // 所以代码可以这么理解Student 继承 父类Person 中的this，而父类Person继承Object的this进行加工
...
* 那么在开发中我们我们需要在一个类中需要用到多个类的方法，应该如何来做呢？这个时候我们可以使用混入（mixin）；
  ...
  stu.mixinsFun() // 混入的方法 
  stu.running() // running
  原理：新创建一个class 这个class继承自Student类，而 new这个新创建的类 就可以使用他的方法，实现混入的效果。
