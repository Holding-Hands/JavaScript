...
3. 计算机中储存的信息都是用二进制数表示的；而我们在屏幕上看到的英文、汉字等字符是二进制数转换之后的结果。通俗的说，按照何种规则将字符存储在计算机中，如'z'用什么表示，称为"编码"；反之，将存储在计算机中的二进制数解析显示出来，称为"解码"，如同密码学中的加密和解密。在解码过程中，如果使用了错误的解码规则，则导致'a'解析成'b'或者乱码。
...
上面规则太多了，我再总结一下，传入两个参数，先比较大小，谁大放后面，谁小放前面，如果其中有为负数，将负数转为0
```
...
```js
trimEnd()方法,从一个字符串的末端移除空白字符。trimRight() 是这个方法的别名。存在浏览器兼容问题不推荐使用
...
#### 25.  trimStart()方法(ES10)

```js
trimStart() 方法从字符串的开头删除空格。trimLeft() 是此方法的别名。存在浏览器兼容问题不推荐使用
...
const height = 180
const name = 'qs'
console.log("my name is " + name + ", age is " + age + ", height is " + height) // my name is qs, age is 18, height is 180

2.es6之后提供模板字符串 ``在其中使用${}中使用变量
// 阅读性更强
const message = `my name is ${name}, age is ${age}, height is ${height} `
console.log(message) // my name is qs, age is 18, height is 180 
...
console.log(`${computedAge()}`) // 36

3. 标签模板字符串的使用(了解即可，react会见到)
function foo (m, n) {
    console.log(m, n)
}
foo('m', 'n') // 正常调用函数
foo`` // [''] undefined 标签模板字符串调用
foo`hello world` // ['hello world'] undefined 是将``里的内容当作第一个参数传入过去，且是一个数组
foo`hello${name}wor${age}ld` //  ['hello', 'wor', 'ld'] 'qs' 相必看到这大家也明白了，使用${}语法 将 原来的 模板字符串分割为三份了hello，wor，ld这三部分将组合称一个数组，按照左到右顺序，作为函数第一个参数，${}里的内容将作为函数剩余参数，此例子中将name的值'qs'传入给函数第二个参数，如果${}中变量是数字，将此数字当作参数传入
foo`hello${11}wor${age}l${11}d` // ['hello', 'wor', 'l', 'd'] 11
```
