...
1. // 有块级作用域每次寻魂执行都会重新开辟一个内存空间来执行，因为定时器中还用到了i，所以没有执行完立即销毁，还是会保留作用域中声明的变量
for(let i = 0; i < 5; i++) {
    setTimeout(() => {
        console.log(i);
    },1000)
...
2.使用let声明的变量是可以改变的
3.使用const声明的变量是不可以改变的（同一作用域下），不可以改变的变量就是常量，所以const主要是用来声明（常量一般大写,也可小写
4.在同一作用域不能重复声明

// 使用const声明的常量在同一作用域不可以修改,let声明大变量是可以修改的
{
  const NAME = 'XM';
  NAME = 'ZCY'; // 这样是会报错的  
}
